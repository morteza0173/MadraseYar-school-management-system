...
import { AddAnnouncementData } from "@/actions/announcementAction";
import { useGetClassDetails } from "@/hooks/useGetClassDetails";

const AddAnnouncementForm = ({ onCancel }: { onCancel: () => void }) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: FormData) => AddAnnouncementData(data),
    onSuccess: (data) => {
      toast.success(data.message || "اعلامیه با موفقیت اضافه شد");
      queryClient.invalidateQueries({ queryKey: ["announcements"] });
      onCancel();
    },
    onError: (error) => {
      toast.error(error.message || "خطا در اضافه کردن اعلامیه");
    },
  });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  اگر کلاسی انتخاب نکنید به عنوان اعلامیه عمومی برای همه ارسال
                  میشود
                </FormDescription>
                <FormMessage />
              </FormItem>
