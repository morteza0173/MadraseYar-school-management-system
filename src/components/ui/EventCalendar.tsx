...
function EventCalendar({
  ...
  React.useEffect(() => {
    const handleResize = () => {
      setIsMdUp(window.innerWidth >= 1024);
    };
  ...
  const RenderDayContent = (day: Date) => {
    ...
    const eventDots = events.slice(0, maxDots).map((event, index) => {
      ...
      return (
        <div
          key={index}
          className={`absolute w-1 h-1 lg:w-2 lg:h-2 rounded-full ${color}`}
      ...
      <div className="relative">
        ...
        <div className="absolute -top-1 -right-2 flex flex-col items-center">
          ...
          {events.length > maxDots && (
            <span className="absolute text-xs text-gray-500 top-4 lg:left-1">
              +{events.length - maxDots}
            </span>
          )}
        </div>
